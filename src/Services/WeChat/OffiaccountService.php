...
use Andruby\Login\Models\WeChat\WechatAutoReply;
use Andruby\Login\Models\WeChat\WechatResponse;
...
class OffiaccountService
{
    ...
    public static function qrcode($appId, $userId)
    {
        try {
            ...
            $wxAuthorization = WxAuthorization::where($where)->first();
            if (!$wxAuthorization) {
                return '';
            }
    ...
    {
        try {
            self::$wxAuthorization = WxAuthorization::where("uuid", $appId)->where("status", 1)->where("type", 0)->first();
